...
struct NativeBridgeRuntimeCallbacks {
    [[maybe_unused]] const char* (*getMethodShorty)(JNIEnv* env, jmethodID mid);
...
DCL_HOOK_FUNC(bool, LoadNativeBridge, const char* nb_library_filename,
    ...
    if (nb_library_filename != std::string_view(ZYGISKLDR)) {
#if defined(__LP64__)
    xumount2(("/system/lib64/"s + nb_library_filename).data(), MNT_DETACH);
#else
    xumount2(("/system/lib/"s + nb_library_filename).data(), MNT_DETACH);
#endif
        return old_LoadNativeBridge(nb_library_filename, runtime_cbs);
    }
    return false;
}
...
void hook_functions() {
    ...
    PLT_HOOK_REGISTER_SYM(art_dev, art_inode,
