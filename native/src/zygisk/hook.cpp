...
DCL_HOOK_FUNC(int, pthread_attr_destroy, void* target) {
    int res = pthread_attr_destroy((pthread_attr_t *)target);
    ...
    // directly call `dlclose` will crash because when `dlcose` returns, PC will reach an unmapped
    // address; so we here use `musttail` to let the compiler resue our stack frame and thus
